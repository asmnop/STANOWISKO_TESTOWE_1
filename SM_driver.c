...
void SM_stop(SM_t *pSM)
{
	...
	pSM->rotation_mode = SM_NONE;
}

void SM_start(SM_t *pSM, const uint8_t direction, const uint16_t freq, const uint16_t steps)
{
	if(pSM->work_mode == SM_STOP)
	{

		if( ((pSM->flag_limit_switch_1 == 1) && (pSM->local_position == pSM->limit_switch_1 + pSM->homing_value) && (direction == SM_CCW) )
		||  ((pSM->flag_limit_switch_2 == 1) && (pSM->local_position == pSM->limit_switch_2 - pSM->homing_value) && (direction == SM_CW) ) )
		{
			return;
		}
		/*
		...
		*/
		...
		pSM->work_mode = SM_get_direction(pSM);
	}
...
void SM_set_steps(SM_t *pSM, const uint8_t direction, const uint16_t freq, const uint16_t steps)
{
	SM_start(pSM, direction, freq, steps);

	if(pSM->work_mode == SM_CW || pSM->work_mode == SM_CCW)
	{
		pSM->rotation_mode = SM_STEP;
	}
}
...
void SM_set_continuous(SM_t *pSM, const uint8_t direction, const uint16_t freq)
{
	...
	SM_start(pSM, direction, freq, 0);

	if(pSM->work_mode == SM_CW || pSM->work_mode == SM_CCW)
	{
		pSM->rotation_mode = SM_CONTINUOUS;
	}
}
...
void SM_set_rampe(SM_t *pSM, const uint8_t direction, const uint16_t time_s, const uint16_t freq, const uint8_t interval)
{
	...
	if(pSM->linear_module_mode == LM_AUTO)
	{
		...
		SM_set_steps(pSM, pSM->param_table[0][0], pSM->param_table[0][1], pSM->param_table[0][2]);
	}
...
void SM_inc_dec_freq(SM_t *pSM, const int16_t value)
{
	...
	if((pSM->work_mode == SM_STOP) || (pSM->rotation_mode == SM_CONTINUOUS))
	{
		if( ((pSM->flag_limit_switch_1 == 1) && (pSM->local_position == pSM->limit_switch_1 + pSM->homing_value) && (SM_get_direction(pSM) == SM_CCW) )
		||  ((pSM->flag_limit_switch_2 == 1) && (pSM->local_position == pSM->limit_switch_2 - pSM->homing_value) && (SM_get_direction(pSM) == SM_CW) ) )
		{
...
void SM_song(SM_t *pSM, SONG_t *pSONG)
{
	...
	if(pSM->linear_module_mode == LM_AUTO)
	{
		...
		//	Sprawdzenie warunku zakresu ruchów wózka:
		uint16_t steps = 0;
		uint16_t step_summary = pSM->homing_value;

		for(uint8_t i=0; i<(number<<1); i = i + 2)
		{
			steps = (((pSONG->notes[i]<<2) * 60) / pSONG->notes[i + 1] ) / pSONG->tempo;

			if(((step_summary - steps) >= pSM->homing_value) && (step_summary - steps) < 10000)
			{
				step_summary = step_summary - steps;
				pSM->param_table[i>>1][0] = SM_CCW;
			}
			else if((step_summary + steps) <= (SM_get_step_range(pSM) - (pSM->homing_value<<1)))
			{
				step_summary = step_summary + steps;
...
void SM_joypad(SM_t *pSM, uint32_t received)
{
	#define IDLE_RANGE	25
	#define CENTER		127
	#define H_LIMIT		(CENTER + IDLE_RANGE)
	#define L_LIMIT		(CENTER - IDLE_RANGE)
	...
	if(pSM->linear_module_mode == LM_MANUAL)
	{
		multipier = 1;
	}
	else
	...
	if(received > CENTER)
	{
		freq = (received - CENTER)<<multipier;
	}
	else if(received < CENTER)
	{
		freq = (CENTER - received)<<multipier;
	}


	if((received > H_LIMIT && pSM->work_mode == SM_STOP))
	{
		SM_set_continuous(pSM, SM_CCW, freq);
		last_received = received;
		flag = 1;
	}
	else if((received < L_LIMIT && pSM->work_mode == SM_STOP))
	{
		SM_set_continuous(pSM, SM_CW, freq);
		last_received = received;
		flag = 1;
	}
	else if(received != last_received && received > H_LIMIT)
	{
		SM_set_freq(pSM, freq);
		last_received = received;
		flag = 1;
	}
	else if(received != last_received && received < L_LIMIT)
	{
		SM_set_freq(pSM, freq);
		last_received = received;
		flag = 1;
	}
	else if(received >= L_LIMIT && received <= H_LIMIT && pSM->work_mode != SM_STANDBY && flag == 1)
	{
		SM_stop(pSM);
		flag = 0;
	}
...
void SM_homing(SM_t *pSM, const uint8_t limit_switch)
{
	#define FREQ	220

	pSM->linear_module_mode = LM_HOMING;

	//	NO LIMIT SWITCH DETECTED:
	if((pSM->flag_limit_switch_1 == 0) && (pSM->flag_limit_switch_2 == 0) && (limit_switch == LIMIT_X1))
	{
		SM_set_continuous(pSM, SM_CW, FREQ);
	...
	else if((pSM->flag_limit_switch_1 == 0) && (pSM->flag_limit_switch_2 == 0) && (limit_switch == LIMIT_X2))
	{
		SM_set_continuous(pSM, SM_CCW, FREQ);
		while(pSM->flag_limit_switch_1 == 0);
		while(pSM->steps_to_make != 0);
	...
	//	ONE LIMIT SWITCH DETECTED:
	else if((pSM->flag_limit_switch_1 == 1) && (pSM->flag_limit_switch_2 == 0) && (limit_switch == LIMIT_X1))
	{
		SM_set_continuous(pSM, SM_CW, FREQ);
		while(pSM->flag_limit_switch_2 == 0);
		while(pSM->steps_to_make != 0);
	...
	else if((pSM->flag_limit_switch_1 == 1) && (pSM->flag_limit_switch_2 == 0) && (limit_switch == LIMIT_X2))
	{
		SM_set_continuous(pSM, SM_CW, FREQ);
		while(pSM->flag_limit_switch_2 == 0);
		while(pSM->steps_to_make != 0);
	...
	else if((pSM->flag_limit_switch_1 == 0) && (pSM->flag_limit_switch_2 == 1) && (limit_switch == LIMIT_X1))
	{
		SM_set_continuous(pSM, SM_CCW, FREQ);
		while(pSM->flag_limit_switch_1 == 0);
		while(pSM->steps_to_make != 0);
	...
	else if((pSM->flag_limit_switch_1 == 0) && (pSM->flag_limit_switch_2 == 1) && (limit_switch == LIMIT_X2))
	{
		SM_set_continuous(pSM, SM_CCW, FREQ);
		while(pSM->flag_limit_switch_1 == 0);
		while(pSM->steps_to_make != 0);
	...
	//	ALL LIMIT SWITCH DETECTED:
	else if((pSM->flag_limit_switch_1 == 1) && (pSM->flag_limit_switch_2 == 1) && (limit_switch == LIMIT_X1))
	{
		SM_set_position(pSM, pSM->homing_value, FREQ);
	}
	else if((pSM->flag_limit_switch_1 == 1) && (pSM->flag_limit_switch_2 == 1) && (limit_switch == LIMIT_X2))
	{
		SM_set_position(pSM, SM_get_step_range(pSM) - pSM->homing_value, FREQ);
	}

	pSM->linear_module_mode = LM_AUTO;
...
void SM_set_rampe(SM_t *pSM, const uint8_t direction, uint16_t time_s, uint16_t freq, uint8_t interval)
{
	//	PRZYKŁAD:
	//	time_s = 250
	//	freq = 800
