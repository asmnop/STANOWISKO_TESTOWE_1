...
typedef struct
{
	...
	uint8_t work_mode;					//	Tryb pracy sterownika: STANDBY, STOP, CW, CCW,
	uint8_t rotation_mode;				//	Tryb pracy - rodzaj obrotów: NONE, STEP, CONTINUOUS,
	uint8_t linear_module_mode;			//	Tryb pracy modułu liniowego,
...
/**
 * @brief Detection of home position and ride linear carriage to them,
 * @note  When home position is detected linear carriage goes to the chosen limit switch,
 * @note  This function cooperate with specific ISR,
 * @param pSM --- Pointer to struct,
 * @param limit_switch --- Limit switch where linear carriage set end position,
 */
void SM_homing(SM_t *pSM, const uint8_t limit_switch);

/**
 * @brief  Calculation range between limit switch positions,
 * @param  pSM --- Pointer to struct,
